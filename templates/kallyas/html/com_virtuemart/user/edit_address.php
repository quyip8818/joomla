...
<form method="post" id="userForm" name="userForm" class="form-validate form-horizontal" action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=user',$this->useXHTML,$this->useSSL) ?>" >
<fieldset>
	<h3 class="m_title upperCase"><?php
	...
	<div class="control-group">
		...
		if (VmConfig::get ('oncheckout_show_register', 1) && $this->userDetails->JUser->id == 0 && $this->address_type == 'BT' and $rview == 'cart') {
			...
			<?php if (!VmConfig::get ('oncheckout_only_registered', 0)) { ?>
				...
				<?php } ?>
			<button class="btn" type="reset" onclick="window.location.href='<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=' . $rview.'&task=cancel'); ?>'"><?php echo vmText::_ ('COM_VIRTUEMART_CANCEL'); ?></button>
			<?php
		}
		else {
			?>
