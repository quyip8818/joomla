...
?>
	...
	<div class="addtocart-area">
		<form method="post" class="product js-recalculate" action="<?php echo JRoute::_ ('index.php?option=com_virtuemart',false); ?>">
			...
			if(!empty($rowHeights['customfields'])) echo shopFunctionsF::renderVmSubLayout('kl_customfields_productpage',array('product'=>$product,'position'=>'addtocart'));
			...
			if (!VmConfig::get('use_as_catalog', 0)  ) { ?>
				...
				if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($product->product_in_stock - $product->product_ordered) < 1) { ?>
					<p>
						<a href="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id=' . $product->virtuemart_product_id); ?>" class="btn btn-info notify"><?php echo vmText::_ ('COM_VIRTUEMART_CART_NOTIFY') ?></a>
					</p>
					<?php
				} else {
					...
					if (!( VmConfig::get('askprice', true) and empty($tmpPrice) ) ) { ?>
						<?php if ($product->orderable) { ?>
							...
							<span>
								<?php echo str_replace('addtocart-button', 'addtocart-button btn btn-danger', $addtoCartButton); ?>
							</span>
						</div>
