...
if ($this->allowRating || $this->allowReview || $this->showRating || $this->showReview) {
	...
	for( $num = 0; $num<=$maxrating; $num++ ) {
		$stars[] = '
				<span title="'.(vmText::_( "COM_VIRTUEMART_RATING_TITLE" ).$num.'/'.$maxrating).'" class="ratingbox dorating">
	...
	if ($review_editable) {
		...
		<form method="post"
			...
			<div class="write-reviews">
				...
				$reviewJavascript = "
		jQuery(function($) {
			jQuery('.rating .ratingbox.dorating > span').mousemove( function(e){
				var span = jQuery(this);
				var countBack = span.prevAll();
				var countStars = countBack.length + 1;
				span.addClass('hovered');
				countBack.addClass('hovered');
				span.nextAll().removeClass('hovered');
				$('#vote').val(countStars);
			});
		});
		";
...
";
				...
				<p
					class="step"><?php echo vmText::sprintf( 'COM_VIRTUEMART_REVIEW_COMMENT', VmConfig::get( 'reviews_minimum_comment_length', 100 ), VmConfig::get( 'reviews_maximum_comment_length', 2000 ) ); ?></p>
				<p>
				<textarea class="input-xxlarge" title="<?php echo vmText::_( 'COM_VIRTUEMART_WRITE_REVIEW' ) ?>"
				...
				<p class="formfooter">
					...
					<input type="text" value="0" size="4" name="counter" class="vm-default input-mini" maxlength="4" readonly="readonly"/>

					<?php
					if($review_editable and $this->allowReview) { ?>
						<input class="btn" type="submit" onclick="return( check_reviewform());"
					...
					<?php } else if($review_editable and $this->allowRating) { ?>
						<input class="btn" type="submit" name="submit_review"
			...
			foreach ($this->rating_reviews as $review) {
				...
				if (!empty($this->rating_reviews) && $review->published) {
					...
					<div class="<?php echo $color ?>">
						...
						<span class="vote">
							<span title="<?php echo vmText::_( "COM_VIRTUEMART_RATING_TITLE" ).$num.'/'.$maxrating ?>" class="ratingbox">
							<?php
								for ($i = 1; $i <= $maxrating; $i++){
									$israted = $i <= (int)$review->review_rating ? 'israted':'';
