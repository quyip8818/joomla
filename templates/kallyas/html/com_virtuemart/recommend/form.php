...
if (empty ( $this->product )) {
	...
	if(empty($this->login) or VmConfig::get('recommend_unauth',false)){
		...
		<div class="ask-a-question-view">
			<h1 class="page-title"><?php echo vmText::_('COM_VIRTUEMART_PRODUCT_ASK_QUESTION')  ?></h1>
			...
			<div class="form-field">
				...
				<form method="post" class="form-validate form-horizontal" action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$this->product->virtuemart_product_id.'&virtuemart_category_id='.$this->product->virtuemart_category_id.'&tmpl=component', FALSE) ; ?>" name="askform" id="askform">
					...
					<div class="control-group">
						<label for="comment" class="control-label"><?php echo vmText::sprintf('COM_VIRTUEMART_ASK_COMMENT', $min, $max); ?></label>
						<div class="controls">
							<textarea title="<?php echo vmText::sprintf('COM_VIRTUEMART_ASK_COMMENT', $min, $max) ?>" class="validate[required,minSize[<?php echo $min ?>],maxSize[<?php echo $max ?>]] field input-xlarge" id="comment" name="comment" rows="8"><?php echo $askQuestionData['comment'] ?></textarea>
						</div>
					</div>
					<div class="control-group">
						<label for="counter" class="control-label"><?php echo vmText::_('COM_VIRTUEMART_ASK_COUNT')  ?></label>
						<div class="controls">
					...
					<div class="control-group">
						<div class="controls">
							...
							?>
							<input class="highlight-button btn btn-primary" type="submit" name="submit_ask" title="<?php echo vmText::_('COM_VIRTUEMART_ASK_SUBMIT')  ?>" value="<?php echo vmText::_('COM_VIRTUEMART_ASK_SUBMIT')  ?>" />
						</div>
					</div>
